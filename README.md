...
For short descriptions of every category see [here](#usage). More details on used PDF-file abbreviations and evaluation metrics can be found in [./eval_results_pdf/evaluation_description.txt](./eval_results_pdf/evaluation_description.txt). The title within every PDF document gives a short description for every evaluation and the 'List of Abbreviations' lists a description for every used evaluation metric. Moreover, titles hold used parameter names for testing. Detailed descriptions can be found in figure titles and captions. Parameter values and options correspond to settings used within [MATCHING- AND POSELIB](https://github.com/josefmaierfl/matchinglib_poselib).
