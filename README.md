...
Evaluations were performed on the provided [MATCHING- AND POSELIB](https://github.com/josefmaierfl/matchinglib_poselib) library using ground truth data generated by [SemiRealSequence](https://github.com/josefmaierfl/SemiRealSequence). These evaluations were focused on the accuracy, runtime, and robustness of various relative pose estimation and refinement algorithms provided with the library. Testing results for various keypoint detectors, descriptor extractors, and matching algorithms can be found in the repository for the [MATCHING- AND POSELIB](https://github.com/josefmaierfl/matchinglib_poselib) library.

Results are provided using figures in multiple PDF documents located at [./eval_results_pdf](./eval_results_pdf) and grouped into 8 main categories:
- Parameter sweep on USAC parameters: [./eval_results_pdf/usac-testing.7z](./eval_results_pdf/usac-testing.7z)
- Comparison of different RANSAC variants: [./eval_results_pdf/usac_vs_ransac.7z](./eval_results_pdf/usac_vs_ransac.7z)
- Comparison of various pose refinement algorithms and cost functions: [./eval_results_pdf/refinement_ba.7z](./eval_results_pdf/refinement_ba.7z)
- Evaluation on correspondence filtering methods in terms of runtime and subsequent pose estimation accuracy: [./eval_results_pdf/vfc_gms_sof.7z](./eval_results_pdf/vfc_gms_sof.7z)
- Comparison of various pose refinement algorithms and cost functions on aggregated and filtered feature correspondences over multiple stereo frames: [./eval_results_pdf/refinement_ba_stereo.7z](./eval_results_pdf/refinement_ba_stereo.7z)
- Comparison of various parameter settings on the feature match aggregation functionality: [./eval_results_pdf/correspondence_pool.7z](./eval_results_pdf/correspondence_pool.7z)
- Parameter sweep on robustness parameters of the continuous high accuracy stereo pose estimation and refinement framework: [./eval_results_pdf/robustness.7z](./eval_results_pdf/robustness.7z)
- Comparison of USAC with simple correspondence aggregation using a windowing approach and the continuous high accuracy stereo pose estimation and refinement framework: [./eval_results_pdf/usac_vs_autocalib.7z](./eval_results_pdf/usac_vs_autocalib.7z)

For short descriptions of every category see [here](#usage). More details on used PDF-file abbreviations and evaluation metrics can be found in [./eval_results_pdf/evaluation_description.txt](./eval_results_pdf/evaluation_description.txt).
A detailed overview on performed evaluations can be found in [./py_test_scripts/Test_Overview/Autocalibration-Parametersweep-Testing.xlsx](./py_test_scripts/Test_Overview/Autocalibration-Parametersweep-Testing.xlsx) and used parameter intervals for testing the [MATCHING- AND POSELIB](https://github.com/josefmaierfl/matchinglib_poselib) library can be found in [./py_test_scripts/Test_Overview/Autokalibrierung_Paramtersweep_Werte.xlsx](./py_test_scripts/Test_Overview/Autokalibrierung_Paramtersweep_Werte.xlsx).

The title within every PDF document gives a short description for every evaluation and the 'List of Abbreviations' lists a description for every used evaluation metric. Moreover, titles hold used parameter names for testing. Detailed descriptions can be found in figure titles and captions. Parameter values and options correspond to settings used within [MATCHING- AND POSELIB](https://github.com/josefmaierfl/matchinglib_poselib).

Due to space restrictions on Github, only most interesting results are provided. For more detailed results, raw results, used GT datasets, statistical results (`*.csv`), and Latex (`*.tex`) files please contact the author at josefjohann-dot-maier-at-gmail-dot-com. Furthermore, additional evaluations were performed for the first 4 points of the listing above using a higher variance in ground truth pose parameters - please contact the author if you are interested in this data.
