...
class SparseDataset(Dataset):
	...
	def __getitem__(self, idx):
		...
		if self.fmat:
			...
			if ratios.shape[1] != pts1.shape[1] or ratios.shape[0] != pts1.shape[0]:
				order = sorted(list(pts1.shape))
				pts1 = np.reshape(pts1, (order[0], order[2], order[1]))
				pts2 = np.reshape(pts2, (order[0], order[2], order[1]))
